...
func TestLocalResolverFactory_Shutdown(t *testing.T) {
	...
	t.Run("All components present", func(t *testing.T) {
		...
		testFactory := &LocalResolverFactory{
			cancelFunc: func() {
				cancelCalled = true
			},
			flagLogger:  mockLogger,
			resolverAPI: nil, // Set to nil for this test
		}

		testFactory.Shutdown(ctx)
	...
	// Test case 2: Nil components (should not panic)
	...
	// Test case 3: Only cancel function
	t.Run("Only cancel function", func(t *testing.T) {
		cancelCalled := false
	...
	// Test case 4: Only flag logger
	t.Run("Only flag logger", func(t *testing.T) {
		mockLogger := &mockWasmFlagLoggerForFactory{}
