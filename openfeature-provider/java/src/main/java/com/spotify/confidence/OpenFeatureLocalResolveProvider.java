...
public class OpenFeatureLocalResolveProvider implements FeatureProvider {
  ...
  public OpenFeatureLocalResolveProvider(
    ...
    this(accountStateProvider, clientSecret, materializationStore, new NoOpWasmFlagLogger());
  }
  ...
  public OpenFeatureLocalResolveProvider(
    ...
    this.wasmResolveApi = new ThreadLocalSwapWasmResolverApi(wasmFlagLogger, materializationStore);
    this.grpcFlagResolver =
        new ConfidenceGrpcFlagResolver(new LocalProviderConfig().getChannelFactory());
  }
  ...
  private CompletableFuture<ResolveFlagsResponse> remoteResolve(ResolveFlagsRequest req) {
    ...
    return grpcFlagResolver.resolve(
        req.getFlagsList(), req.getClientSecret(), req.getEvaluationContext());
  }
