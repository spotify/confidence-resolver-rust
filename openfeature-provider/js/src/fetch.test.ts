...
// Helpers
function makeSink(spy:(url:string, init?:RequestInit) => Response | Promise<Response>) : Fetch {
  return async (url:string, init?:RequestInit) => {
    return await spy(url, init);
  }
}
...
describe('Fetch.create composition', () => {
  it('wraps middlewares right-to-left', async () => {
    ...
    const sink = makeSink(() => new Response(null, { status: 200 }));
...
describe('withTimeout', () => {
  it('aborts the request after timeout (or skips if unsupported)', async () => {
    const sink = makeSink((_url, init) => new Promise<Response>((_resolve, reject) => {
      init?.signal?.addEventListener('abort', () => reject(init.signal?.reason));
    }));
    const f = Fetch.create([withTimeout(15)], sink);
    const p = f('http://example.test');
    const guard = new Promise<never>((_r, reject) => setTimeout(() => reject(new Error('timeout waiting for abort')), 500));
...
describe('withRetry', () => {
  it('retries on 5xx and eventually succeeds', async () => {
    const calls:number[] = [];
    const sink = makeSink((_url) => {
      calls.push(Date.now());
      return calls.length === 1
        ? new Response(null, { status: 500 })
        : new Response(null, { status: 200 });
    });
    const f = Fetch.create([withRetry({ baseDelayMs: 1, jitter: 0 })], sink);
  ...
  it('uses Retry-After header when provided (HTTP-date small delta)', async () => {
    ...
    const f = Fetch.create([withRetry({ baseDelayMs: 1, maxDelayMs: 10_000, jitter: 0 })], sink);
  ...
  it('replays a ReadableStream body across retries', async () => {
    const seen:string[] = [];
    let attempt = 0;
    const sink = makeSink(async (_url, init) => {
  ...
  it('aborts during backoff if signal aborted', async () => {
    ...
    const f = Fetch.create([withRetry({ baseDelayMs: 10, jitter: 0 })], sink);
...
describe('withAuth', () => {
  it('adds Authorization header and retries once on 401 with renewed token', async () => {
    ...
    let nextStatus = 401;
    const sink = makeSink((_url, init) => {
      calls.push({ auth: (init?.headers instanceof Headers) ? init.headers.get('Authorization') ?? undefined : new Headers(init?.headers as any).get('Authorization') ?? undefined });
...
describe('withRouter', () => {
  it('matches simple trailing * (prefix match)', async () => {
    const sink = makeSink(() => new Response('ok'));
  ...
  it('returns 404 when no route matches', async () => {
    const sink = makeSink(() => new Response('base'));
    const routed = Fetch.create([
      withRouter({ 'https://api.example.com/v1/*': [next => next] })
    ], sink);
