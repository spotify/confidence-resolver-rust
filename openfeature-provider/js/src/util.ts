...
export function scheduleWithFixedInterval(operation:(signal?:AbortSignal) => unknown, intervalMs:number, opt: { maxConcurrent?:number, signal?:AbortSignal } = {}):() => void {
...
export function portableSetTimeout(callback:() => void, milliseconds:number, opt:{ unref?:boolean } = {}):number {
  ...
  if(opt.unref && typeof timeout === 'object' && timeout !== null && 'unref' in timeout && typeof timeout.unref === 'function') {
    timeout.unref();
  }
